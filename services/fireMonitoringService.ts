...
export interface FireWeatherProduct { id: number; index_type: string; forecast_day: number; product_kind?: 'forecast' | 'historical_reanalysis'; valid_at: string; status: IntelligenceStatus; quality_state: string; statistics?: { min: number; max: number; mean: number; count: number }; warnings?: string[] | null; artifact_urls?: { geojson: string; cog: string } | null; }
