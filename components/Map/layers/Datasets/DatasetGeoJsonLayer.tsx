...
function categoryForFeature(layer: DatasetLayer, feature: GeoJSON.Feature): DatasetLayerCategoryStyle | null {
  ...
  if (style.renderer !== 'categorized' || !style.categorized?.field) {
    return null;
  }

  const properties = (feature.properties || {}) as Record<string, unknown>;
  const value = String(properties[style.categorized.field] ?? '');

  return style.categorized.categories.find((category) => String(category.value) === value) || null;
}

function visualColor(layer: DatasetLayer, feature: GeoJSON.Feature): string {
  const category = categoryForFeature(layer, feature);

  return category?.fillColor
    || category?.color
    || layer.style.markerColor
    || layer.style.color
    || layer.style.fillColor
    || '#60a5fa';
}

function pathStyleForFeature(layer: DatasetLayer, feature: GeoJSON.Feature, pane: string): L.PathOptions {
  const style = layer.style || {};
  const category = categoryForFeature(layer, feature);

  if (style.renderer === 'none' || category?.enabled === false) {
    return {
      pane,
      opacity: 0,
      fillOpacity: 0,
  ...
  const categoryColor = category?.color || category?.fillColor;
  const categoryFill = category?.fillColor || category?.color;

  return {
    pane,
    color: categoryColor || style.color || style.markerColor || '#2563eb',
    fillColor: categoryFill || style.fillColor || style.color || '#93c5fd',
    fillOpacity: category?.fillOpacity ?? style.fillOpacity ?? (layer.geometry_family === 'polygon' ? 0.22 : 0),
    opacity: category?.opacity ?? style.opacity ?? 0.86,
    weight: category?.weight ?? style.weight ?? (layer.geometry_family === 'line' ? 2 : 1.4),
  };
}
...
function tooltipHtml(layer: DatasetLayer, feature: GeoJSON.Feature): string {
  ...
  const color = visualColor(layer, feature);
...
export const DatasetGeoJsonLayer: React.FC<DatasetGeoJsonLayerProps> = ({
  ...
  return (
    <GeoJSON
      key={`${layer.id}-${bbox}-${filterKey}-${styleKey}-${refreshKey}`}
      data={featureCollection as any}
      pane={pane}
      style={(feature) => pathStyleForFeature(layer, feature as GeoJSON.Feature, pane)}
      pointToLayer={(feature, latlng) => {
        const category = categoryForFeature(layer, feature as GeoJSON.Feature);
        const hidden = layer.style.renderer === 'none' || category?.enabled === false;

        return L.circleMarker(latlng, {
          pane,
          radius: hidden ? 0 : layer.style.radius || 5,
          color: category?.color || layer.style.strokeColor || '#0f172a',
          weight: hidden ? 0 : category?.weight ?? 1.5,
          fillColor: category?.fillColor || category?.color || layer.style.markerColor || layer.style.color || '#2563eb',
          fillOpacity: hidden ? 0 : category?.fillOpacity ?? layer.style.opacity ?? 0.92,
          opacity: hidden ? 0 : layer.style.opacity ?? 0.92,
        }) as any;
      }}
