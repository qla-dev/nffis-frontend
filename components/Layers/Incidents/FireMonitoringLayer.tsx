...
const EXTREME_TEST_FIRE: GeoJSON.Feature<GeoJSON.Point, FireEventProperties> = {
  ...
  properties: {
    ...
    weather: {
      ...
      from: 'W',
      towards: 'E',
      provider: 'Synthetic FWI test mode',
    },
    entity: 'Federation of Bosnia and Herzegovina',
    canton: 'Herzegovina-Neretva Canton',
    municipality: 'Čapljina / Hutovo Blato test point',
  },
};

export function FireMonitoringLayer({
  visible,
  showSpreadWarnings = false,
  includeRecentExtinguished = true,
  injectExtremeTestFire = false,
  onEventsLoaded,
}: FireMonitoringLayerProps) {
  const map = useMap();
  const layerRef = useRef<L.LayerGroup | null>(null);

  useEffect(() => {
    if (!visible && !showSpreadWarnings) { if (layerRef.current) map.removeLayer(layerRef.current); layerRef.current = null; return; }
    let alive = true;
    ...
    const load = async () => {
      ...
      try {
        let collection;
        try {
          collection = await fetchActiveFires(includeRecentExtinguished, controller.signal);
        } catch (error) {
          if ((error as Error).name === 'AbortError' || !injectExtremeTestFire) throw error;
          // Keep the visual test self-contained when the FireWatch API is offline.
          collection = { type: 'FeatureCollection' as const, features: [] };
        }
        if (!alive) return;
        if (layerRef.current) map.removeLayer(layerRef.current);
        const group = L.layerGroup();
        const features = injectExtremeTestFire
          ? [...collection.features, EXTREME_TEST_FIRE]
          : collection.features;
        onEventsLoaded?.(features.map((feature) => feature.properties as FireEventProperties));
  ...
  }, [includeRecentExtinguished, injectExtremeTestFire, map, onEventsLoaded, showSpreadWarnings, visible]);
